...
- User-visible capability: 提供统一的 PAOS state-file envelope；对 `TARGETS.md`、`SESSIONS.md`、`ENVIRONMENT.md` 提供严格的 schema/shadow/preview 边界。`SKILLRUNTIME.md` 与 Skill-scoped `LESSONS.md` 不由本 adapter 生成。
...
- `ENVIRONMENT.md`: strict `projection` mode; atomic writes and digest-based drift checks
- `SKILLRUNTIME.md`: no production producer in this feature; Runtime manifest/state remains authoritative
- Skill-scoped `LESSONS.md`: generated by the Experience ledger producer, not this adapter
...
- authority-boundary conformance: `tests/test_state_file_authority_boundaries.py` covers SQLite migration/concurrent origin deduplication, immutable origins, full-record update revalidation, manifest tamper/partial-write prevention, strict robot-state JSON, Bundle identity binding, ENVIRONMENT prompt isolation, writer-owned artifact paths, and retention mutation protection
