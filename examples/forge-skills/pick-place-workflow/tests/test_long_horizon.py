...
def test_skill_dag_projects_dependencies_without_execution():
    ...
    assert [node.node_id for node in WORKFLOW_DAG.ready_nodes({"observe"})] == [
        "capabilities", "understand",
    ]
    assert [node.node_id for node in WORKFLOW_DAG.ready_nodes({"observe", "capabilities"})] == [
        "understand",
    ]
    assert [node.node_id for node in WORKFLOW_DAG.ready_nodes({"observe", "understand"})] == [
        "capabilities",
    ]
    assert [node.node_id for node in WORKFLOW_DAG.ready_nodes({"observe", "capabilities", "understand"})] == [
        "propose",
    ]
    ...
    assert WORKFLOW_DAG.nodes[5].resource_requirement.mode.value == "alternative_resource"
    assert WORKFLOW_DAG.nodes[5].resource_requirement.substitution_allowed is True


def test_agent_can_choose_either_independent_query_before_propose_join():
    workflow = LongHorizonWorkflow.start("task-1", "revision-1")
    workflow.record("observe", "available", {"observation_ref": OBS})
    assert workflow.next_tools() == ("manipulation.capabilities", "scene.understand")

    workflow.record("understand", "available", {"observation_ref": OBS})
    assert workflow.next_tools() == ("manipulation.capabilities",)
    workflow.record("capabilities", "available", {
        "observation_ref": OBS,
        "capability_snapshot_ref": CAP,
    })
    assert workflow.next_tools() == ("grasp.propose",)
...
def test_complete_chain_requires_terminal_success_and_preserves_references():
    workflow = LongHorizonWorkflow.start("task-1", "revision-1")
    workflow.record("observe", "available", {"observation_ref": OBS})
    workflow.record("capabilities", "available", {"observation_ref": OBS, "capability_snapshot_ref": CAP})
    workflow.record("understand", "available", {"observation_ref": OBS})
    workflow.record("propose", "available", {"observation_ref": OBS, "capability_snapshot_ref": CAP, "candidate_set_ref": CSET})
    workflow.record(
        "prepare",
        "available",
        {"observation_ref": OBS, "candidate_set_ref": CSET, "preparation_ref": PREP, "capability_snapshot_ref": CAP},
    )
    workflow.record(
        "acquire",
        "succeeded",
    ...
    assert state.steps[5].references["invocation_ref"] == "invocation://object-acquire/a1"
...
def test_prepare_acquire_and_place_require_their_bound_references():
    ...
    workflow.record("understand", "available", {"observation_ref": OBS})
    with pytest.raises(WorkflowBindingError):
        workflow.record("propose", "available", {"observation_ref": OBS, "capability_snapshot_ref": CAP})
    workflow.record("propose", "available", {"observation_ref": OBS, "capability_snapshot_ref": CAP, "candidate_set_ref": CSET})
    with pytest.raises(WorkflowBindingError):
        workflow.record("prepare", "available", {"observation_ref": OBS, "candidate_set_ref": CSET, "capability_snapshot_ref": CAP})
    workflow.record("prepare", "available", {"observation_ref": OBS, "candidate_set_ref": CSET, "preparation_ref": PREP, "capability_snapshot_ref": CAP})
    with pytest.raises(WorkflowBindingError):
...
def test_terminal_response_adapter_preserves_place_evidence_and_identity():
    ...
    workflow.record_terminal_response("capabilities", {"status": "available", "result": {"observation_ref": OBS, "capability_snapshot_ref": CAP}})
    workflow.record_terminal_response("understand", {"status": "available", "result": {"observation_ref": OBS}})
    workflow.record_terminal_response("propose", {"status": "available", "result": {"observation_ref": OBS, "candidate_set_ref": CSET, "capability_snapshot_ref": CAP}})
    workflow.record_terminal_response("prepare", {"status": "available", "result": {"observation_ref": OBS, "candidate_set_ref": CSET, "preparation_ref": PREP, "capability_snapshot_ref": CAP}})
...
def test_recovery_is_append_only_and_terminal_workflow_cannot_be_rewritten():
    ...
    assert workflow.state.steps[2].status == "blocked" or workflow.state.steps[2].status == "failed"
